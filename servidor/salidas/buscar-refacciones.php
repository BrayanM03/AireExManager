<?php
    ...
    if($_POST){
     ...
     date_default_timezone_set('America/Matamoros');
        ...
        $consultar = "SELECT COUNT(*) FROM refacciones WHERE descripcion LIKE ? OR marca LIKE ? OR modelo LIKE ? AND sucursal = ?";
        $resp = $con->prepare($consultar);
        $resp->execute([$input, $input, $input, $sucursal]);
        $total = $resp->fetchColumn();
        $resp->closeCursor();

       if($total>0){
        $input = '%'.$_POST["input"].'%';
        $consultar = "SELECT * FROM refacciones WHERE descripcion LIKE ? OR marca LIKE ? OR modelo LIKE ? AND sucursal = ?";
        $resp = $con->prepare($consultar);
        $resp->execute([$input, $input, $input, $sucursal]);
