<?php
    ...
    $contar = "SELECT COUNT(*) FROM detalle_cotizacion_tmp WHERE producto_id = ? AND user_id = ? AND categoria = ?";
    ...
    $insert = "INSERT INTO detalle_cotizacion_tmp(id,
                                            descripcion,
                                            cantidad,
                                            precio_unitario,
                                            importe,
    ...
    $response = array("status"=> true, "mensj"=>"Los datos se insertaron correctamente");
        ...
        $contar = "SELECT * FROM detalle_cotizacion_tmp WHERE producto_id = ? AND user_id = ? AND categoria =?";
        $re = $con->prepare($contar);
        $re->execute([$id_producto, $_SESSION["id"], $categoria]);
        ...
        $nueva_cantidad = $cantidad + $cantidad_actual;
            ...
            $update = "UPDATE detalle_cotizacion_tmp SET cantidad = ?, importe = ? WHERE producto_id = ? AND user_id = ? AND categoria =?";
